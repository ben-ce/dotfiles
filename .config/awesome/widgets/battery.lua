...
awesome.connect_signal("signal::battery", function(percentage, state, time_to_empty, time_to_full, kind)
  ...
  if kind == 0 then
    bat_icon = '  '
    battery_icon.font = "JetBrains Mono Nerd Font 12"
    battery_icon.markup = "<span foreground='" .. beautiful.green .. "'>" .. bat_icon .. "</span>"
    battery_tooltip.text = string.format('%s', "Unknown device, likely running on AC power.")
  else
