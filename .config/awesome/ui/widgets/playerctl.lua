...
local create_button = function(symbol, color, command, playpause)
  ...
  local icon = wibox.widget {
    markup = helpers.ui.colorize_text(symbol, color),
  ...
  playerctl:connect_signal("playback_status", function(_, playing, _)
    if playpause then
      if playing then
        icon.markup = helpers.ui.colorize_text("", color)
      else
        icon.markup = helpers.ui.colorize_text("", color)
      end
    end
  ...
  button:connect_signal("mouse::enter", function()
    icon.markup = helpers.ui.colorize_text(icon.text, beautiful.foreground)
  end)

  button:connect_signal("mouse::leave", function()
    icon.markup = helpers.ui.colorize_text(icon.text, color)
  end)
...
playerctl:connect_signal("metadata",
  function(_, title, artist, art_path, _, _, _)
    ...
    title_widget:set_markup_silently(
      '<span foreground="' .. beautiful.color5 .. '">' .. title .. '</span>')
    artist_widget:set_markup_silently(
      '<span foreground="' .. beautiful.color6 .. '">' .. artist .. '</span>')
  end)
...
local playerctl_play_symbol = create_button("", beautiful.color4,
  play_command, true)

local playerctl_prev_symbol = create_button("玲", beautiful.color4,
  prev_command, false)
local playerctl_next_symbol = create_button("怜", beautiful.color4,
  next_command, false)
...
local slider = wibox.widget {
  ...
  background_color = beautiful.color0 .. 55,
  color = beautiful.color6,
  value = 25,
  max_value = 100,
