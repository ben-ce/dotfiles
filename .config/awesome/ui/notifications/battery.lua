...
awesome.connect_signal("signal::battery", function(percentage, state)
	...
	if value <= 15 and display_low and state == 2 then
		naughty.notification({
			...
			app_name = "AwesomeWM",
			image = gears.color.recolor_image(icons.battery_low, beautiful.err_color),
		})
		display_low = false
	end
	...
	if display_high and state == 4 and value > 90 then
		naughty.notification({
			...
			text = "Fully charged!",
			app_name = "AwesomeWM",
			image = gears.color.recolor_image(icons.battery, beautiful.battery_charging_color),
		})
		display_high = false
	end
	...
	if display_discharge and state == 2 then
		naughty.notification({
			...
			app_name = "AwesomeWM",
			image = gears.color.recolor_image(icons.battery, beautiful.fg_normal),
		})
	...
	if display_charge and state == 1 then
		naughty.notification({
			...
			text = "Charging",
			app_name = "AwesomeWM",
			image = gears.color.recolor_image(icons.charging, beautiful.battery_charging_color),
		})
