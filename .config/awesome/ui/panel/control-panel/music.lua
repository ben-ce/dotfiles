...
playerctl:connect_signal("metadata", function(_, title, artist, album_path, _, __, player_name)
	...
	if player_name:match("mpd") then
		music_display:buttons(gears.table.join(awful.button({}, 1, function()
			dash_toggle()
			awful.spawn(user_likes.music, false)
		end)))
	elseif player_name ~= "None" then
		music_display:buttons(gears.table.join(awful.button({}, 1, function()
			dash_toggle()
			awful.spawn(string.lower(player_name), false)
		end)))
	end
	...
	music_display:get_children_by_id("music_text_role")[1].markup =
		helpers.ui.colorize_text(player_name, beautiful.bg_normal)
end)
